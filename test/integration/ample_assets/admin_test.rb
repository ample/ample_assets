...
module AmpleAssets
  class AdminTest < ActionDispatch::IntegrationTest
    ...
    context 'The Asset Toolbar' do
      ...
      context 'working with documents' do
        
        setup do
          visit '/ample_assets/files/new'
          attach_file 'Attachment', ::Rails.root.join('app/assets/sample.pdf')
          click_button 'Create File'
          visit new_page_path
          fill_in 'Title', :with => 'Test Page with PDF'
          click_button 'Create Page'
          click_link 'Test Page with PDF'
          click_link 'Assets'
        end

        should 'allow users to drag an asset into an assocation' do
          assert page.find_by_id('page_file_id').value.blank?
          image = page.find_by_id('recent-assets').find('a.document')
          drop = page.find('.asset-drop .droppable')
          image.drag_to(drop)
          assert !page.find_by_id('page_file_id').value.blank?
        end
        
        should 'display a document icon in lieu of a preview when associated' do
          test_page = Page.find_by_title('Test Page with PDF')
          test_page.file_id = File.last.id
          test_page.save
          test_page.reload
          visit edit_page_path(test_page)
          assert page.find('.asset-drop .droppable div.document')
          assert !page.find('.asset-drop .droppable img')
        end
